...
const guestIsolation = index.indexOf("guest-world-isolation.js?v=0.4.1");
...
const appIndex = index.indexOf("app.js?v=0.4.15");
assert.ok(guestIsolation >= 0 && guestIsolation < persistenceIndex && persistenceIndex < storeIndex && storeIndex < appIndex, "persistence must load immediately after guest isolation and before all canonical world writers");
...
const context = vm.createContext({ window, localStorage: first, Object, String, queueMicrotask(callback) { callback(); }, console });
