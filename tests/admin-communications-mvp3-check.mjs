...
assert.deepEqual(resolveSystemRecipients(worldState, "ALL", "").characterIds.sort(), ["a", "b", "c", "d"]);
...
await adminCommunicationsHandler(request("POST", {
  ...
  fetchImpl: async (url, options) => {
    ...
    if (url.includes("baekji_admin_system_send")) {
      sentSystemBody = body;
      return okJson([{ id: 9, login_id: "AD1", display_name: "관리자 AD1", target_kind: "CHARACTER", target_id: "b", target_label: "캐릭터 B", message: body.p_message, recipient_count: 1, session_count: 1, created_at: "2026-08-10T00:00:01Z" }]);
    }
    throw new Error(`unexpected ${url}`);
  },
});
...
await playerAdminSystemHandler(request("GET", null, { url: "/api/player-admin-system?characterId=b&after=7", cookie: "" }), playerFeed, {
  ...
  fetchImpl: async (_url, options) => {
    ...
    return okJson([{ id: 9, admin_name: "관리자 AD1", target_kind: "CHARACTER", target_label: "캐릭터 B", message: "즉시 뒤를 확인하세요.", recipient_session_ids: ["s1"], created_at: "2026-08-10T00:00:01Z" }]);
  },
});
...
assert.match(index, /admin-system-feed\.js\?v=0\.3\.80/);
assert.match(vercel, /\/api\/admin-communications/);
assert.match(vercel, /\/api\/player-admin-system/);

console.log("PASS: MVP3 admin chat, scoped operator SYSTEM delivery, private character targeting, and player emphasis are wired without mutating world state");
