...
assert.match(index, /cloud-state-sync\.js\?v=0\.4\.5&fix=0b1&movement-terminal=1&result-party-disband=1&stage4-item-transfer=1&item-disposition=1&field-item-management=1&stage6c-ingress=1/, "movement cloud reconciliation must ship behind a fresh browser cache key");
assert.match(index, /app\.js\?v=0\.4\.15&fix=0b1[^"']*movement-terminal=1[^"']*transfer-privacy=1[^"']*movement-departure-presence=1[^"']*item-disposition=1[^"']*stage5-world-store=1[^"']*stage6a=1/, "movement terminal markers must ship behind the exact fresh cache key");
