...
const inspectCommand = appSource.slice(appSource.indexOf("async function inspectObject"), appSource.indexOf("async function takeItem"));
assert.match(inspectCommand, /PLAYER_WORLD_COMMANDS__\.dispatch\("INVESTIGATION_ACTION_V1", \{ sessionId, kind: "INSPECT", targetId: objectId \}\)/);
assert.match(inspectCommand, /\[\'APPLIED\', \'REPLAY\'\]\.includes\(result\?\.status\)/, "object media opens only after the authoritative inspection command settles");
assert.match(inspectCommand, /showObjectModal\(objectId\)/);
