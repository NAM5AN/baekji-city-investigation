...
assert(!index.includes("foundation-rule-fixes.js"), "retired browser movement rules must not load beside the authoritative command reducer");
assert.match(index, /app\.js\?v=0\.4\.18[^"']*stage3a=1[^"']*stage3b=1[^"']*stage3c=1[^"']*transfer-privacy=1[^"']*movement-departure-presence=1[^"']*item-disposition=1[^"']*stage5-world-store=1[^"']*stage6a=1[^"']*stage8b=1[^"']*stage8b-b5=1/);
assert(index.indexOf("player-world-commands.js") < index.indexOf("app.js?v=0.4.18"), "authoritative command boundary must load before app.js starts");
assert(index.includes("tester-auth.js?v=0.3.90&stage6b=1&stage8b=1"), "tester auth cache key must be refreshed");
assert(index.includes("tester-login-stable.js?v=0.3.97&stage6b=1&stage8b=1"), "stable tester login owner must be loaded");
