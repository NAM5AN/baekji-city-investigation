(() => {
  ...
  function systemHistoryMarkup() {
    ...
    return rows.length ? rows.map((row) => `<article class="admin-system-history-row">
      <header><strong>운영 SYSTEM</strong><span>${esc(timeText(row.created_at))}</span></header>
      <p>${esc(row.message || "")}</p>
  ...
  async function openSystemComposer() {
    ...
    try {
      ...
      renderSystemComposer(snapshot, { kind: "ALL", targetId: "", message: "", confirm: false });
    } catch {
  ...
  function renderSystemComposer(snapshot, draft) {
      ...
      root.querySelector("[data-admin-system-confirm-send]")?.addEventListener("click", async (event) => {
        ...
        try {
          const data = await request(COMM_API, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ kind: "system", targetKind: kind, targetId, targetLabel, message: draft.message }),
          });
          if (data.event) {
    ...
    root.querySelectorAll("[data-admin-system-kind]").forEach((button) => button.addEventListener("click", () => renderSystemComposer(snapshot, { kind: button.dataset.adminSystemKind, targetId: "", message: root.querySelector("[data-admin-system-message]")?.value || "", confirm: false })));
    root.querySelector("[data-admin-system-target]")?.addEventListener("change", (event) => renderSystemComposer(snapshot, { kind, targetId: event.target.value, message: root.querySelector("[data-admin-system-message]")?.value || "", confirm: false }));
    root.querySelector("[data-admin-system-review]")?.addEventListener("click", () => {
      ...
      renderSystemComposer(snapshot, { kind, targetId, message, confirm: true });
    });
  }
